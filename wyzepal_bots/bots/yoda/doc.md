...
This directory contains library code for running WyzePal
...
inside a WyzePal repo:

    cd ~/wyzepal/api
    wyzepal-run-bot yoda --config-file ~/.wyzepalrc-prod
...
with using the WyzePal API, but here is a quick review of
what a `.wyzepalrc` files looks like.  You can connect to the
API as your own human user, or you can go into the WyzePal settings
page to create a user-owned bot.

    [api]
    email=someuser@example.com
    key=<your api key>
    site=https://wyzepal.somewhere.com
